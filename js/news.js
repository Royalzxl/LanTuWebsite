...
(function ($) {
    ...
    $.fn.extend({
        "pageBean" : function (options) {
            ...
            this.each(function () {
                ...
                //为每一页的页码绑定事件
                $this.find(opts.pageClass).children("li").on('click', function () {
                    $(this).addClass("select").siblings().removeClass("select");
                    var nowPage = parseInt($(this).text());
                    addNewpage(nowPage);
                });
                //触发默认第一页的点击时间
                $this.find(opts.pageClass).children("li").eq(opts.nowPage - 1).trigger('click');
                //为当前页数传入内容
                function addNewpage(nowPage) {
                    $this.find(".n-m-w-box").remove();
                    ...
                    for (var j = 0; j < nowPageArr.length; j++){
                        $(".n-m-w-box").eq(j).find(".img img").attr("src", nowPageArr[j].imgUrl);
                        $(".n-m-w-box").eq(j).find(".text a").attr("href", nowPageArr[j].url);
                        $(".n-m-w-box").eq(j).find(".text a").text(nowPageArr[j].title);
                        $(".n-m-w-box").eq(j).find(".text span").text(nowPageArr[j].time);
                        $(".n-m-w-box").eq(j).find(".text p").text(nowPageArr[j].detail);
                    }
                }
            })
        }
    ...
    /**
     *  根据总的list和每页数目以及当前页数来确定当页的list为什么
     * @param arr
     * @param pgNum
     * @param perPageNum
...
$(function () {
    ...
    $(".last").on('click', function () {
        $(".ul li:last-child").trigger('click');
    });
    $(".prev").on('click', function () {
        $(".list.select").prev().trigger('click');
    });
    $(".next").on('click', function () {
        $(".list.select").next().trigger('click');
    })
});